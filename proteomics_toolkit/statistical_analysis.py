...
def _apply_log_transformation_if_needed(data, config):
    ...
    if config.log_transform_before_stats == "auto":
        ...
        if hasattr(config, 'normalization_method') and config.normalization_method:
            ...
            if already_log_transformed:
                ...
                print(f"Log transformation: AUTO-DETECTED (not needed - {config.normalization_method} already log-transforms data)")
            else:
                apply_log_transform = True
                print(f"Log transformation: AUTO-DETECTED (needed - {config.normalization_method} preserves original scale)")
        else:
    ...
    print(f"  -> New data range: {transformed_data[numeric_columns].min().min():.2f} to {transformed_data[numeric_columns].max().max():.2f}")
...
class StatisticalConfig:
    ...
    def __init__(self):
        ...
        self.enable_pvalue_fallback = True     # Auto-fallback to unadjusted if no adjusted significant
...
def run_paired_t_test(protein_data, metadata_df, config):
    ...
    print(f"Running paired t-test analysis...")
    ...
    for i, (protein_idx, protein_values) in enumerate(protein_data.iterrows()):
        ...
        protein_df = pd.DataFrame({
            'Sample': protein_values.index,
            'Intensity': protein_values.values
        })
...
def run_mixed_effects_analysis(protein_data, metadata_df, config, protein_annotations=None):
    ...
    print(f"Running mixed-effects analysis...")
    print(f"  Model: Protein ~ {' * '.join(config.interaction_terms)} + (1|{config.subject_column})")
    ...
    for i, (protein_idx, protein_values) in enumerate(protein_data.iterrows()):
        ...
        try:
            ...
            formula = f"Intensity ~ {all_interaction_terms[0]} * {all_interaction_terms[1]}"
            ...
            group_candidates = [
                p for p in params.index 
                if config.interaction_terms[0] in p and ':' not in p and p != 'Intercept'
            ]
            ...
            time_candidates = [
                p for p in params.index
                if config.interaction_terms[1] in p and ':' not in p and p != 'Intercept'
            ]
...
def run_unpaired_t_test(protein_data, metadata_df, config):
    ...
    print(f"Running unpaired t-test analysis...")
...
def apply_multiple_testing_correction(results_df, config):
    ...
    rejected, adj_pvalues, _, _ = multipletests(all_pvalues, method=config.correction_method)
...
def run_comprehensive_statistical_analysis(normalized_data, sample_metadata, config, protein_annotations=None):
    ...
    if len(normalized_data.columns) > 5:
        sample_columns = list(normalized_data.columns[5:])  # Everything after first 5 annotation columns
        print(f"  Using standardized data structure: {len(sample_columns)} sample columns (columns 6+)")
    else:
        # Fallback for legacy data (shouldn't happen with create_standard_data_structure)
        sample_columns = normalized_data.select_dtypes(include=[np.number]).columns.tolist()
    ...
    group_pairs = {group: [p for p in complete_pairs if p['group'] == group] for group in config.group_labels}
    
    print(f"  Complete paired subjects by group:")
    ...
    if config.statistical_test_method == "mixed_effects":
        ...
        raise ValueError(f"Unknown statistical method: {config.statistical_test_method}")
        ...
        if available_cols:
            ...
            for col in display_df.columns:
                if col in ['P.Value', 'adj.P.Val']:
                    display_df[col] = display_df[col].apply(lambda x: f"{x:.2e}" if pd.notna(x) and x < 0.01 else f"{x:.6f}" if pd.notna(x) else "N/A")
