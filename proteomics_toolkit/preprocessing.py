...
def create_standard_data_structure(data: pd.DataFrame, cleaned_sample_names: Dict[str, str] = None) -> pd.DataFrame:
    ...
    print(f"Sample column range: {sample_columns[0] if sample_columns else 'None'} ... {sample_columns[-1] if sample_columns else 'None'}")
    ...
    print(f"Sample columns ({len(sample_part)}): {sample_part[0] if sample_part else 'None'} ... {sample_part[-1] if sample_part else 'None'}")
    
    # Check that annotation columns are exactly what we expect
    if annotation_part != required_annotation_cols:
        raise ValueError(f"Annotation columns don't match expected order. Got: {annotation_part}, Expected: {required_annotation_cols}")
...
def assess_data_completeness(data: pd.DataFrame, sample_columns: List[str], 
                           sample_metadata: Dict[str, Dict]) -> None:
    """
...
def filter_proteins_by_completeness(data: pd.DataFrame, sample_columns: List[str], 
                                   min_detection_rate: float = 0.5) -> pd.DataFrame:
    """
...
def calculate_group_colors(sample_metadata: Dict[str, Dict]) -> Tuple[Dict[str, str], pd.Series]:
    """
    ...
    if meaningful_groups:
        ...
        study_colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', 
                       '#e377c2', '#17becf', '#bcbd22']  # Standard colors for study groups
...
def identify_annotation_columns(data: pd.DataFrame) -> List[str]:
    ...
    annotation_patterns = [
        'protein', 'gene', 'description', 'name', 'accession', 'entry',
        'symbol', 'uniprot', 'database', 'organism'
    ]
...
def classify_samples(sample_metadata: Dict, group_column: str, group_labels: List[str],
                    control_column: str, control_labels: List[str], 
                    apply_systematic_colors: bool = True,
                    systematic_color_palette: str = "Set1") -> Tuple[Dict, List[str], List[str], Dict, Optional[Dict]]:
    """
    ...
    return group_distribution, control_samples, study_samples, corrected_sample_metadata, group_colors


def apply_systematic_color_scheme(sample_metadata: Dict, group_labels: List[str], control_labels: List[str],
                                systematic_color_palette: str, use_systematic_colors: bool = True) -> Tuple[Dict, pd.Series]:
    """
    ...
    if use_systematic_colors:
        ...
        for i, group in enumerate(config_ordered_groups):
            if i < len(high_contrast_colors):
                ...
                systematic_group_colors[group] = high_contrast_colors[i % len(high_contrast_colors)]        # Create ordered group list: study groups first, then control pools
        control_groups = []
        study_groups = []
