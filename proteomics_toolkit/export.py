...
def export_analysis_results(
    ...
    if annotation_cols is None:
        ...
        annotation_cols = ['Protein', 'Description', 'Protein Gene', 'UniProt_Accession', 'UniProt_Entry_Name']
    ...
    if (differential_results is not None and 
        not differential_results.empty and 
...
def _add_annotations_to_results(
    ...
    annotation_first_cols = [col for col in available_annotation_cols 
                           if col in differential_results_annotated.columns]
    statistical_cols = [col for col in differential_results_annotated.columns 
...
def _add_annotations_to_normalized_data(
    ...
    if len(normalized_data) != len(filtered_data):
        raise ValueError(f"Mismatch: normalized_data has {len(normalized_data)} rows, filtered_data has {len(filtered_data)} rows")
    ...
    annotation_count = len([col for col in ['Protein', 'Description', 'Protein Gene', 'UniProt_Accession', 'UniProt_Entry_Name'] 
                           if col in result_data.columns])
    
    print(f"✓ Final column order: {annotation_count} annotations + {len(sample_cols)} samples")
    print("✓ CLEAN structure: No redundant or scattered columns!")
    print(f"✓ Column order: {list(result_data.columns[:5])}... + {len(sample_cols)} sample columns")
    print("✓ NO redundant columns like 'Gene', 'UniProt_Database', extra 'Description' etc.")
...
def _display_results_preview(annotated_results: pd.DataFrame) -> None:
    ...
    if len(annotated_results) > 0:
        ...
        preview_cols = [col for col in ['Protein', 'Gene', 'UniProt_Accession', 'logFC', 'P.Value', 'adj.P.Val'] 
...
def export_timestamped_config(
    ...
    with open(config_file, 'w', encoding='utf-8') as f:
        ...
        section_configs = [
            (1, "INPUT FILES AND PATHS", ['toolkit_path', 'metadata_file', 'protein_file', 'peptide_file', 'remove_common_prefix']),
            (2, "DATA FILTERING PARAMETERS", ['min_detection_rate']),
            (3, "NORMALIZATION STRATEGY", ['normalization_method', 'optimize_vsn']),
            (4, "NEGATIVE VALUE HANDLING STRATEGY", ['handle_negatives', 'negative_handling_method', 'min_positive_replacement']),
            (5, "STATISTICAL ANALYSIS STRATEGY", ['statistical_test_method', 'analysis_type']),
            (6, "EXPERIMENTAL DESIGN CONFIGURATION", ['subject_column', 'paired_column', 'paired_label1', 'paired_label2', 'group_column', 'group_labels']),
            (7, "MIXED-EFFECTS MODEL CONFIGURATION", ['interaction_terms', 'additional_interactions', 'covariates']),
            (8, "CONTROL SAMPLE CONFIGURATION", ['control_column', 'control_labels']),
            (9, "VISUALIZATION SETTINGS", ['use_systematic_colors', 'systematic_color_palette', 'group_order', 'group_colors']),
            (10, "SIGNIFICANCE THRESHOLDS", ['p_value_threshold', 'fold_change_threshold', 'q_value_max', 'use_adjusted_pvalue', 'enable_pvalue_fallback']),
            (11, "OUTPUT AND EXPORT SETTINGS", ['export_results', 'output_prefix', 'label_top_proteins', 'random_seed', 'min_samples_per_group'])
        ]
        
        for section_num, section_name, param_names in section_configs:
            _write_config_section(f, section_name, config_dict, param_names, section_num)
...
def _write_config_section(file_handle, section_name: str, config_dict: Dict[str, Any], param_names: List[str], section_number: int = 1) -> None:
...
def _print_export_summary(exported_files: Dict[str, str], config_file: str) -> None:
    ...
    print("\n" + "="*60)
    print("✓ All analysis results and configuration exported successfully!")
    print("Files created:")
...
def export_significant_proteins_summary(
    ...
    summary_cols = ['Protein', 'Gene', 'UniProt_Accession', 'Description', 
...
def export_results(differential_df: pd.DataFrame, output_file: str, 
