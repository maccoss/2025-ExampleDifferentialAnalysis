...
def plot_box_plot(data: pd.DataFrame, sample_columns: List[str], 
                 sample_metadata: Dict[str, Dict], 
                 group_colors: Optional[Dict[str, str]] = None,
                 group_order: Optional[List[str]] = None,
                 log_transform: bool = True,
                 figsize: Tuple[int, int] = (16, 8),
                 title: str = "Protein Intensity Distribution by Sample") -> None:
    """
...
def plot_volcano(differential_df: pd.DataFrame, 
                ...
                normalization_method: Optional[str] = None) -> None:
    """
    ...
    if use_adjusted_pvalue == "adjusted" and 'adj.P.Val' in df.columns:
        ...
        if significant_count > 0 or not enable_pvalue_fallback:
            ...
            fallback_used = True
            print(f"⚠️  No significant proteins found using adjusted p-values (FDR < {p_threshold})")
            print(f"📊 Automatically falling back to unadjusted p-values for visualization")
            print(f"    Note: Results shown use raw p-values, interpret with caution")
        else:
            p_col_used = 'adj.P.Val'
            p_type_label = "FDR"
    ...
    for color in ['gray', 'orange', 'blue', 'red']:
        ...
        if len(subset) > 0:
            label = {'gray': 'Not significant', 'orange': 'Significant', 
                    'blue': 'Decreased', 'red': 'Increased'}[color]
            ax.scatter(subset['logFC'], subset['neg_log10_p'], 
    ...
    if label_top_n > 0 and gene_column in df.columns:
        ...
        ].sort_values(p_col_used).head(label_top_n)
        
        for _, row in significant.iterrows():
            ax.annotate(row[gene_column], 
    ...
    n_up = len(df[(df['logFC'] > fc_threshold) & 
                  (df[p_col_used] < p_threshold)])
    n_down = len(df[(df['logFC'] < -fc_threshold) & 
                    (df[p_col_used] < p_threshold)])
...
def plot_normalization_comparison(original_data: pd.DataFrame, 
    ...
    print(f"Range reduction: {1 - (norm_medians.max() - norm_medians.min()) / (original_medians.max() - original_medians.min()):.1%}")


def plot_sample_correlation_heatmap(data: pd.DataFrame, sample_columns: List[str],
                                   sample_metadata: Dict[str, Dict],
                                   figsize: Tuple[int, int] = (12, 10),
                                   method: Literal['pearson', 'kendall', 'spearman'] = 'pearson',
                                   group_colors: Optional[Dict[str, str]] = None) -> None:
    """
    ...
    with warnings.catch_warnings():
        ...
        g = sns.clustermap(correlation_matrix, 
                          ...
                          vmin=actual_min, vmax=1.0,  # Scale from actual minimum to perfect correlation
                          square=True,
                          linewidths=0.1,
        ...
        g.fig.suptitle(f'Sample Correlation Heatmap ({method.title()})', 
                       fontsize=16, y=1.02)
...
def plot_sample_correlation_triangular_heatmap(data: pd.DataFrame, sample_columns: List[str],
                                               sample_metadata: Dict[str, Dict],
    ...
    ax.set_title(f'Sample Correlation Heatmap - Triangular{clustering_text}\n{method.title()} Correlation', 
...
def plot_pca(data: pd.DataFrame, sample_columns: List[str],
            sample_metadata: Dict[str, Dict],
            group_colors: Optional[Dict[str, str]] = None,
    ...
    for group in unique_groups:
        ...
        ax.scatter(pca_result[group_indices, 0], pca_result[group_indices, 1], 
                  c=group_colors[group], label=group, alpha=0.7, s=100, 
...
def plot_comparative_pca(original_data: pd.DataFrame, 
    ...
    def perform_pca_analysis(data, title_suffix, log_transform=True):
        ...
        pca_df = pd.DataFrame(pca_result[:, :2], 
                            columns=['PC1', 'PC2'],
                            index=complete_data.columns)
        
        # Add group information
        pca_df['Group'] = [sample_metadata.get(sample, {}).get('Group', 'Unknown') 
    ...
    def print_pca_summary(pca, pca_df, dataset_name):
        ...
        if len(unique_groups) > 1:
            ...
            for group in unique_groups:
                group_data = pca_df[pca_df['Group'] == group]
                if len(group_data) > 0:
                    group_centroids[group] = (group_data['PC1'].mean(), group_data['PC2'].mean())
            ...
            if len(group_centroids) > 1:
                ...
                for i in range(len(groups)):
                    for j in range(i+1, len(groups)):
                        ...
                        dist = np.sqrt((group_centroids[g1][0] - group_centroids[g2][0])**2 + 
...
def plot_control_correlation_analysis(original_data: pd.DataFrame,
                                     median_normalized_data: pd.DataFrame,
                                     vsn_normalized_data: pd.DataFrame,
                                     sample_columns: List[str],
                                     sample_metadata: Dict[str, Dict],
    ...
    if control_labels is None:
        ...
        control_patterns = ['Pool', 'Control', 'QC', 'Standard', 'Blank', 'pool', 'control', 'qc', 'standard', 'blank']
        control_labels = []
    ...
    corr_original = create_control_heatmap(original_data, 'Control Samples Correlation\n(Original Data)', axes[0], log_transform=True)
    corr_median = create_control_heatmap(median_normalized_data, 'Control Samples Correlation\n(Median Normalized)', axes[1], log_transform=True)
    corr_vsn = create_control_heatmap(vsn_normalized_data, 'Control Samples Correlation\n(VSN Normalized)', axes[2], log_transform=False)
...
def plot_control_group_correlation_analysis(data: pd.DataFrame,
                                          sample_columns: List[str],
    ...
    for _, (group_name, group_samples) in enumerate(valid_groups.items()):
        ...
        fig.suptitle(f'Correlation Matrix - {group_name} ({normalization_method} Normalized)', 
        ...
        for row in range(n_samples):
            for col in range(n_samples):
                ...
                if row == col:
                    ...
                    correlation = corr_matrix.loc[sample_y, sample_x]
                    ax.text(0.5, 0.5, f'{correlation:.3f}***' if correlation > 0.95 else f'{correlation:.3f}', 
...
def plot_individual_control_pool_analysis(data: pd.DataFrame,
                                         sample_columns: List[str],
                                         sample_metadata: Dict[str, Dict],
                                         control_column: str = "Subject",
                                         control_labels: Optional[List[str]] = None,
    ...
    for _, (pool_name, pool_sample_list) in enumerate(valid_pools.items()):
        ...
        # Create correlation matrix plot with pairwise scatter plots
        fig, axes = plt.subplots(n_samples, n_samples, figsize=figsize)
        fig.suptitle(f'Correlation Matrix - {pool_name} ({normalization_method} Normalized)', 
                    fontsize=16, fontweight='bold')
