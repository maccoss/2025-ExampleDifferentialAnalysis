...
def _create_standardized_column_order(annotation_data: pd.DataFrame, sample_data: pd.DataFrame) -> List[str]:
    ...
    if actual_cols != expected_annotation_cols:
        raise ValueError(f"Annotation data must have standardized columns.\n"
...
def _separate_sample_and_annotation_data(data: pd.DataFrame, sample_columns: Optional[list] = None) -> tuple:
    ...
    if len(data.columns) < 5:
        raise ValueError(f"Data must have at least 5 columns for standardized structure. Got {len(data.columns)} columns. "
    ...
    if actual_annotation_cols != expected_annotation_cols:
        raise ValueError(f"Data does not have standardized annotation structure.\n"
                        f"Expected: {expected_annotation_cols}\n"
...
def vsn_normalize(data: pd.DataFrame, optimize_params: bool = False, sample_columns: Optional[list] = None) -> pd.DataFrame:
    """
    ...
    if annotation_data is not None:
        ...
        result = pd.concat([annotation_data.reset_index(drop=True), 
                           normalized_sample_data.reset_index(drop=True)], axis=1)
...
def quantile_normalize(data: pd.DataFrame, sample_columns: Optional[list] = None) -> pd.DataFrame:
    ...
    normalized_sample_data = pd.DataFrame(normalized_matrix, 
...
def log_transform(data: pd.DataFrame, base: str = 'log2', 
...
def mad_normalize(data: pd.DataFrame, sample_columns: Optional[list] = None) -> pd.DataFrame:
    ...
    for col in sample_data.columns:
        if sample_mads.loc[col] > 0:  # Avoid division by zero
            normalized_sample_data[col] = ((sample_data[col] - sample_medians[col]) / sample_mads.loc[col]) * global_mad + global_median
...
def z_score_normalize(data: pd.DataFrame, sample_columns: Optional[list] = None) -> pd.DataFrame:
    ...
    for col in sample_data.columns:
        if sample_stds.loc[col] > 0:  # Avoid division by zero
            normalized_sample_data[col] = ((sample_data[col] - sample_means[col]) / sample_stds.loc[col]) * global_std + global_mean
...
def calculate_normalization_stats(data: pd.DataFrame, normalized_data: pd.DataFrame, 
    ...
    if method.lower() != 'vsn':
        log2_original = pd.DataFrame(np.log2(data.replace(0, np.nan)), 
                                   index=data.index, columns=data.columns)
        log2_normalized = pd.DataFrame(np.log2(normalized_data.replace(0, np.nan)), 
                                     index=normalized_data.index, columns=normalized_data.columns)
    else:
        log2_original = pd.DataFrame(np.log2(data.replace(0, np.nan)), 
                                   index=data.index, columns=data.columns)
...
def calculate_detailed_normalization_stats(original_data: pd.DataFrame, normalized_data: pd.DataFrame, 
                                         method: str, sample_metadata: dict = None,
                                         control_labels: list = None) -> dict:
    """
    ...
    if method.lower() == 'vsn':
        ...
        original_for_stats = pd.DataFrame(np.log2(original_data.replace(0, np.nan)), 
    ...
    overall_stats = {
        ...
        'normalized_cv_median_all_samples': (normalized_for_stats.std(axis=0) / normalized_for_stats.mean(axis=0)).median(),
    }
    ...
    if sample_metadata and control_labels:
        for control_type in control_labels:
            ...
            control_samples = [sample for sample, meta in sample_metadata.items() 
            ...
            if len(control_samples) >= 2:  # Need at least 2 samples to calculate CV
                ...
                orig_cvs = (orig_control_data.std(axis=1) / orig_control_data.mean(axis=1)).replace([np.inf, -np.inf], np.nan)
                norm_cvs = (norm_control_data.std(axis=1) / norm_control_data.mean(axis=1)).replace([np.inf, -np.inf], np.nan)
...
def handle_negative_values(data: pd.DataFrame, method: str = "min_positive", 
