...
class TestCreateStandardDataStructure:
    ...
    def test_structure_with_cleaned_sample_names(
        self, processed_data, cleaned_sample_names
    ):
    ...
    def test_annotation_column_content(self, processed_data):
        ...
        assert list(result["UniProt_Entry_Name"]) == [
            "TRFE_HUMAN",
            "KLK10_HUMAN",
            "LIPL_HUMAN",
        ]
    ...
    def test_column_order_is_preserved(self, processed_data):
        ...
        for col in sample_cols:
            assert col not in annotation_col_names, (
                f"Sample column section contains annotation column: {col}"
            )
        ...
        peptide_data = pd.DataFrame(
            {
                ...
                "Peptide": [
                    "PEPTIDEONE",
                    "PEPTIDETWO",
                ],  # Additional peptide-specific column
